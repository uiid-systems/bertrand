...
function printUsage() {
  console.log(`
bertrand sync — replicate this machine's database to Supabase Storage (opt-in)

Usage:
  bertrand sync onboard        Interactive setup for this machine
  bertrand sync invite         Print a paste-able bundle for another machine
  bertrand sync <bundle>       Decode bundle, save config, pull
  bertrand sync push           VACUUM → encrypt → upload
  bertrand sync pull [--force] Download → decrypt → atomic-replace
  bertrand sync status         Local + remote object size and timestamps
  bertrand sync enable         Turn auto-triggers on
  bertrand sync disable        Turn auto-triggers off

There is one database (~/.bertrand/bertrand.db) and one set of credentials
(~/.bertrand/sync.env), so there is nothing to target: the old --project flag
selected among per-project databases that no longer exist.
...
async function runOnboard() {
  ...
  console.log(`Bertrand sync setup for this machine\n`);
  ...
  // One database, one object. The default used to be `projects/<slug>/…` so
  // several per-project databases could share a bucket without colliding;
  // there is nothing left to collide with, and the flat key is what a second
  // machine importing an invite will pull.
  ...
  if (pasted) {
    ...
    console.log(`other machine that should be able to pull this database:\n`);
  ...
  console.log(`\nNext:`);
  console.log(`  bertrand sync push      Send this machine's DB to Supabase`);
...
async function runPush() {
  if (!hasSyncConfig()) return abort("sync is not configured — run `bertrand sync onboard`");
...
function runEnable() {
  if (!hasSyncConfig()) {
    return abort("sync is not configured — run `bertrand sync onboard` first");
  }
  patchConfig({ sync: { enabled: true } });
...
function runInvite() {
  ...
  console.log(
    "Sensitive: this bundle contains a Supabase service_role token AND the database's encryption key."
  );
  console.log(
  ...
  console.log(`  (saves config, then replaces that machine's database with this one)`);
}

async function runBootstrap(invite: string) {
  console.log("Importing from invite…");
  ...
  console.log(`Wrote ${paths.syncEnv} and pulled into ${result.dbPath}.`);
  if (result.pulled) {
    console.log(
