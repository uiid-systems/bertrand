 ...
 * it: they ran the command, so the answers to "which project?" and "should
 * this be recorded at all?" are already given. Auto-adoption has nobody to
 * ask, so this command is `runAdopt` plus the two gates
 * `docs/session-identity.md` requires it to pass before it may create
 * anything:
 *
 *   1. The cwd resolves to a registered project, **by git origin**. A
 *      directory belonging to no project is not ours to record — we refuse,
 *      and never invent a project to hold it.
 *   2. That project has `autoAdopt` on. Off is the default, so a machine that
 *      upgrades into this feature behaves exactly as it did before until
 *      someone opts a repo in.
 *
 ...
 * every prompt in every unregistered directory on the machine would pay for
...
import { resolveProjectForCwd } from "@/lib/projects/policy";
...
export type AutoAdoptDecline =
  ...
  | "already-launched"
  /** The cwd resolves to no registered project. */
  | "no-project"
  /** It resolves to one, but that project has not opted in. */
  | "not-opted-in"
  /** The conversation belongs to a session that is archived or deleted. */
  | "archived";

export type AutoAdoptOutcome =
  | { ok: true; created: boolean; sessionId: string; slug: string; project: string }
 ...
 * change the answer.
 */
function declineMessage(reason: AutoAdoptDecline, cwd: string, slug?: string): string {
  switch (reason) {
    ...
    case "no-project":
      return (
        `No project owns ${cwd}.\n` +
        `  Sessions are matched to a project by git origin. Link one with:\n` +
        `    bertrand project link <slug> ${cwd}`
      );
    case "not-opted-in":
      return (
        `Project "${slug}" has not opted in to recording sessions bertrand didn't launch.\n` +
        `  Turn it on with: bertrand project auto ${slug} on`
      );
    case "archived":
...
export async function runAutoAdopt(
  ...
  const decline = (reason: AutoAdoptDecline, slug?: string): AutoAdoptOutcome => {
    markAutoCreateDeclined(opts.claudeSessionId, reason);
    return { ok: false, reason, message: declineMessage(reason, opts.cwd, slug) };
  };
  ...
  const project = await resolveProjectForCwd(opts.cwd);
  if (!project) return decline("no-project");
  if (project.autoAdopt !== true) return decline("not-opted-in", project.slug);

  // Pin the project before any DB work. `runAdopt` resolves through
  // `resolveActiveProject()`, which without this would answer with whichever
  // project the user last ran `bertrand project switch` on — a silent
  // cross-project write with no error and no visible symptom.
...
Record the claude session running in this directory, if its project has opted
in. Fired by the UserPromptSubmit hook; the flags exist for testing it by hand.
...
register("auto-adopt", async (args) => {
  ...
  if (args.includes("--json")) {
    ...
    console.log(
      outcome.created
        ? `Recorded this claude session as ${outcome.slug} in ${outcome.project}.`
        : `Re-attached this claude session to ${outcome.slug} in ${outcome.project}.`,
    );
  }

  // Always exit 0. The caller is a hook whose non-zero exits Claude Code
  // surfaces to the user, and "this directory isn't opted in" is the normal
  // state of nearly every directory on the machine, not an error.
});
